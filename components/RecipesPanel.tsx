...
export default function RecipesPanel({ householdId, userId, products }: Props) {
  ...
  return <section className="dashboard-card recipes-card">
    <div className="card-heading"><div><span className="card-label">УМНЫЙ ПОДБОР</span><h2>Что приготовить?</h2></div><span className="count-badge">{recipes.length}</span></div>
    ...
    {!recipes.length && <div className="empty-state">Рецептов пока нет. Конструктор рецептов добавим следующим этапом.</div>}
    <div className="recipe-grid">
      {visible.map(({ recipe, ingredients: recipeIngredients, missing, score }) => <article className="recipe-card" key={recipe.id}>
        ...
        <div className="recipe-ingredients">{recipeIngredients.map((ingredient) => { const available = products.some((product) => matches(product.name, ingredient.name)); return <span className={`ingredient ${available ? "available" : ingredient.optional ? "optional" : "missing"}`} key={ingredient.id}>{available ? "✓" : ingredient.optional ? "○" : "+"} {ingredient.name}</span>; })}</div>
