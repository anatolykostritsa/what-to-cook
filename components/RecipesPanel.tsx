...
import { assessIngredient, normalizeName } from "@/lib/domain/inventory";
...
export default function RecipesPanel({ householdId, userId, products }: Props) {
  ...
  const scored = useMemo(() => recipes.map((recipe) => { const list = ingredients.filter((item) => item.recipe_id === recipe.id); const required = list.filter((item) => !item.optional); const assessments = new Map(list.map((item) => [item.id, assessIngredient(item, products)])); const missing = required.filter((item) => !assessments.get(item.id)!.enough); const covered = required.reduce((sum, item) => { const result = assessments.get(item.id)!; if (result.needed === null) return sum + (result.enough ? 1 : 0); return sum + (result.needed === 0 ? 1 : Math.min(1, (result.available ?? 0) / result.needed)); }, 0); return { recipe, ingredients: list, assessments, missing, score: required.length ? Math.round((covered / required.length) * 100) : 100 }; }), [recipes, ingredients, products]);
  const visible = scored.filter(({ recipe, score }) => (!search.trim() || normalizeName(recipe.name).includes(normalizeName(search))) && (!onlyAvailable || score === 100)).sort((a, b) => b.score - a.score || Number(Boolean(b.recipe.is_favorite)) - Number(Boolean(a.recipe.is_favorite)));
