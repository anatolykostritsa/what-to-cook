...
export default function RecipesPanel({ householdId, userId, products }: Props) {
  ...
  async function addMissingToShopping(recipeId: string) { const item = scored.find((entry) => entry.recipe.id === recipeId); if (!item?.missing.length) return; setAdding(recipeId); const { error: insertError } = await supabase.from("shopping_items").insert(item.missing.map((ingredient) => ({ household_id: householdId, name: ingredient.name, quantity: ingredient.quantity, unit: ingredient.unit, done: false, added_from_recipe: recipeId, created_by: userId }))); if (insertError) setError(insertError.message); setAdding(null); }
